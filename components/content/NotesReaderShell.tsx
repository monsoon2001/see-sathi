    ...
    <ReaderShell>
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-6 lg:px-8">
        <div className="mx-auto mb-4 flex max-w-3xl flex-wrap items-center justify-between gap-2">
          <nav className="flex items-center gap-1 text-label-md font-label-md text-on-surface-variant">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-[1rem] bg-tertiary-fixed text-on-tertiary-fixed shadow-sm">
            ...
            <span className="text-outline-variant">/</span>
            <span className="font-title text-on-surface">{chapter.title.split(" (")[0]}</span>
          </nav>
        ...
        <div className="flex w-full justify-center">
          <article
            className={cn(
              "mx-auto w-full max-w-3xl space-y-10 px-4 sm:px-0",
              !focus && "lg:col-span-9",
            )}
          >
            <header className="scroll-mt-28 space-y-2 pb-4">
              <div className="inline-flex items-center gap-1 rounded-full bg-tertiary/10 px-4 py-2 text-tertiary">
              ...
              <h1 className="text-headline-lg tracking-tight text-on-surface">{chapter.title}</h1>
            ...
            <footer className="flex flex-col items-center justify-between gap-3 border-t border-surface-container pt-6 sm:flex-row">
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full bg-surface-container px-5 py-2.5 font-title text-title text-on-surface transition-colors hover:bg-surface-container-high"
              >
                <ArrowLeft className="h-[18px] w-[18px]" aria-hidden="true" /> Back to Chapter
              </Link>
              <Link
                href={`/subjects/${subject.slug}/${chapter.slug}/questions`}
                className="inline-flex items-center gap-2 rounded-full bg-primary-container px-5 py-2.5 font-title text-title text-on-primary shadow-[0_4px_14px_rgba(75,79,242,0.28)] transition-all hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(75,79,242,0.38)]"
              >
